    ...
    return <Box sx={{
        ...
        <Box sx={{
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Fab size="small" onClick={rewind}
                    disabled={props.camState.current.speed <= -16}>
                    <FastRewindIcon />
                </Fab>
            </Box>
            <FormControl sx={{ height: 32, flexShrink: 1 }}>
                <InputLabel>Car</InputLabel>
                <Select value={camCar?.number} sx={{ width: "100%", height: "100%" }} label="Car" size="small" onChange={(ev) => {
                ...
                }}>
                    {
                        sortedCars.map((car) => <MenuItem value={car.number} key={car.idx}>
                            <Box sx={{
                                ...
                                <Avatar sx={{ width: 16, height: 16, fontSize: 12, bgcolor: car.class.color, color: "black" }} variant="rounded">
                                    {car.number}
                                </Avatar>
                                <Typography noWrap variant="subtitle2" sx={{ flexGrow: 1 }}>{car.driverName}</Typography>
                            </Box>
                        </MenuItem>)
                    }
                </Select>
        ...
        <Box sx={{
            ...
            <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                <Fab size="small" onClick={fastForward} disabled={props.camState.current.speed >= 16}>
                    <FastForwardIcon />
                </Fab>
            ...
            <FormControl sx={{ height: 32 }}>
                <InputLabel>Camera</InputLabel>
                <Select value={props.camState.current.cameraGroupNum} sx={{ width: "100%", height: "100%" }} label="Camera" size="small"
                    onChange={(ev) => {
                    ...
                    {
                        props.camState.cameraGroups.map((group) => <MenuItem value={group.num} key={group.num}>
